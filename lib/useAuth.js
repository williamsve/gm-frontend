...
export function AuthProvider({ children }) {
  ...
  const checkAuth = useCallback(async () => {
    ...
    try {
      ...
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}${API_ENDPOINTS.auth.me}`
  ...
  const login = useCallback(async (username, password) => {
    ...
    try {
      ...
      const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}${API_ENDPOINTS.auth.login}`
  ...
  const loginJson = useCallback(async (username, password) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/auth/login/json`, {
        method: 'POST',
        headers: {
  ...
  const logout = useCallback(async () => {
    try {
      // Llamar directamente a FastAPI (puerto 8000)
      const token = getToken()
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}${API_ENDPOINTS.auth.logout}`, {
        method: 'POST',
        headers: token ? {
          'Authorization': `Bearer ${token}`,
        } : {},
  ...
  const register = useCallback(async (userData) => {
    ...
    try {
      // Llamar directamente a FastAPI (puerto 8000)
      const token = getToken()
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}${API_ENDPOINTS.auth.register}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
